...
bool isPromo(ChessMove m){
    return m.piece == Pawn && ((m.color == White && m.dst <= 7) || (m.color == Black && m.dst >=56)); 
}
    ...
    void processMessages() {
        ...
        while ((pos = _receiveBuffer.find('\n')) != std::string::npos) {
            ...
            if (payload.substr(0, 9) == "TEST:FEN:") {
                ...
                if (_game != nullptr) {
                    _game->FENtoBoard(testFen);
                    _game->updateAI();
                    ChessMove move = _game->getLastAIMove();
                    if (move.piece != NoPiece) {
                        std::string moveStr = "TEST:MOVE:" + std::to_string(static_cast<int>(move.src)) +
                                              "," + std::to_string(static_cast<int>(move.dst));
...
void TournamentClient::sendAIMove() {
    ...
    std::string moveStr = "MOVE:" + std::to_string(static_cast<int>(move.src)) +
                          "," + std::to_string(static_cast<int>(move.dst));
